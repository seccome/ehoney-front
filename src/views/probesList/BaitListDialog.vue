<template>
  <el-dialog title="诱饵列表" visible width="1000px" :close-on-click-modal="false" :before-close="close" v-loading="loading">
    <PageContent>
          ...
          <el-table slot="list" :data="tableData" width="100%" :border="false">
            ...
            <el-table-column label="操作" width="100px">
              <template v-slot="{row}">
                ...
                <el-button v-if="row.BaitType==='FILE' && row.Status > 1" type="primary" size="mini" icon="el-icon-download" circle @click="downloadFile(row)"></el-button>
              </template>
            </el-table-column>
    ...
    <!-- 新建蜜罐诱饵弹窗 -->
    <AddBaitDialog v-if="isShowAddDialog" :honeypotID="ServerID" @close="isShowAddDialog=false" @save="search" />
  </el-dialog>
</template>
...
export default {
  ...
  data () {
    return {
      searchUrl: this.$apis.baitProbeSearch,
      apiUrl: this.$apis.baitProbeCreate,
      searchParams: {
