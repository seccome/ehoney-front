  ...
  <div>
    <PageContent>
      <div slot="form" class="page-form">
        ...
        <form-item label="诱饵名称">
          <el-input @keyup.enter.native="getTableData" v-model="baitName" placeholder="请输入诱饵名称"></el-input>
        </form-item>
        <form-item label="操作系统">
          <select-opt v-model="baitSysType" select-type="getsystype"></select-opt>
        </form-item>
      ...
      <div class="page-main">
        <ListLayout>
          <el-table slot="list" :data="tableData" width="100%" height="100%" :border="false">
            ...
            <el-table-column label="操作系统" prop="systype"></el-table-column>
            ...
            <el-table-column label="操作">
              <div slot-scope="scope">
                <el-button type="text" @click="baitdelete(scope.row)">删除</el-button>
              </div>
            </el-table-column>
    ...
    <Upload title="新建诱饵" v-if="uploadVisible" :visible.sync="uploadVisible" @close="close"></Upload>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getTableData() {
      ...
      try {
        const { baitType, baitName, baitSysType, creator, timeValue, } = this

        const res = await this.$Server('/getbaits', 'POST', {
          baitType, baitName, baitSysType, creator,
    ...
    async baitdelete(item) {
      if (this.loading) return
      this.loading = true

      try {

        await this.$confirm('是否确认删除此诱饵', '删除诱饵', {
          type: 'warning',
        ...
        const { baitid } = item
        const res = await this.$Server('/baitdelete', 'POST', {
          baitId: baitid
        })
